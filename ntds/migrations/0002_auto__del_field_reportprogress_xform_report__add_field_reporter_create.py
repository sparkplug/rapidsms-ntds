...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'ReportProgress.xform_report'
        db.delete_column('ntds_reportprogress', 'xform_report_id')

        # Adding M2M table for field xform_reports on 'ReportProgress'
        m2m_table_name = db.shorten_name('ntds_reportprogress_xform_reports')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('reportprogress', models.ForeignKey(orm['ntds.reportprogress'], null=False)),
            ('xformreportsubmission', models.ForeignKey(orm['rapidsms_xforms.xformreportsubmission'], null=False))
        ))
        db.create_unique(m2m_table_name, ['reportprogress_id', 'xformreportsubmission_id'])

        # Adding field 'Reporter.created'
        db.add_column('ntds_reporter', 'created',
                      self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.datetime.now(), blank=True),
                      keep_default=False)

        # Adding field 'Reporter.updated'
        db.add_column('ntds_reporter', 'updated',
                      self.gf('django.db.models.fields.DateTimeField')(auto_now=True, default=datetime.datetime.now(), blank=True),
                      keep_default=False)


    def backwards(self, orm):
        # Adding field 'ReportProgress.xform_report'
        db.add_column('ntds_reportprogress', 'xform_report',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['rapidsms_xforms.XFormReportSubmission']),
                      keep_default=False)

        # Removing M2M table for field xform_reports on 'ReportProgress'
        db.delete_table(db.shorten_name('ntds_reportprogress_xform_reports'))

        # Deleting field 'Reporter.created'
        db.delete_column('ntds_reporter', 'created')

        # Deleting field 'Reporter.updated'
        db.delete_column('ntds_reporter', 'updated')
