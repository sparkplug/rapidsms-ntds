...
function action2(elem, action,page) {
...
function select_all() {
    if ($('#input_select_all').prop('checked')) {
        $('#span_select_everything').show();
        $('input:checkbox[name=results]').prop('checked', 'checked');
    } else {
        $('#span_select_everything').hide();
        $('input:checkbox[name=results]').prop('checked', false);
    }
}
